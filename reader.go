...
//
// This version of csv.Reader is patched to report whether a column was quoted.
//
...
parseField:
			...
			for {
				...
				if i >= 0 {
					...
					case rn == r.Comma:
						...
						line = line[commaLen:]
						r.fieldIndexes = append(r.fieldIndexes, len(r.recordBuffer)|quoteBit)
						continue parseField
	...
	for i, idx := range r.fieldIndexes {
		quoted := idx&quoteBit == quoteBit
		idx &= ^quoteBit
		dst[i] = Column{
			Quoted: quoted,
			Value:  str[preIdx:idx],
		}
		preIdx = idx
	}
